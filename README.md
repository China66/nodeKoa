...
async findUser(id,user_name,password,is_admin){ // user_name,password,is_admin可以不用传递
    try{
      ...
      ponst res = userRegister.findOne({
...
...
const userServeTest = new userServe()
userServeTest.updateById({1,"小小","654321",true}) // id为1的用户数,修改的用户名字，用户密码，设置为管理员
```
在终端，执行：node src/serve/user.serve.js，结果可以先看终端是否报错，在看数据库的管理工具的数据表是否已经更新，如果有就证明已经更新成功，如果没有，看打印出来的错误提示
